...
//    • environment ZFORTH_AGENT=1  (alias: FORTH_AGENT=1)
//
//  Usage examples:
//    ZForth --agent -e '2 2 + .'
//    ZForth --agent -f /path/to/script.fth -o /tmp/out.txt
//    ZForth --agent --cwd ~/Documents/XCodeProjects/ZForth -e ': 1+ 1 + ; 5 1+ .'
//    ZForth --agent --repl < commands.txt
//    ZFORTH_AGENT=1 ZForth -e 'WORDS'
//
//  Exit status:
...
enum AgentChannel {
    ...
    static var isRequested: Bool {
        ...
        if env["ZFORTH_AGENT"] == "1" || env["FORTH_AGENT"] == "1" {
            return true
        }
    ...
    static func run() -> Int32 {
        ...
        if let cwd = parsed.cwd {
            ...
            if fm.changeCurrentDirectoryPath(cwd) {
                ...
                appendOut("[ZForth agent] cwd \(cwd)\n")
            } else {
                appendOut("[ZForth agent] ERROR: cannot chdir \(cwd)\n")
        ...
        if startSt != 0 {
            appendOut("[ZForth agent] FATAL: kernel cold start failed (\(startSt))\n")
            writeTranscriptIfNeeded(parsed.outPath, transcript)
            ForthCBridge.agentSink = nil
            return 1
        }
        ...
        if parsed.autoload {
            appendOut("[ZForth agent] AutoLoad…\n")
            if let auto = Bundle.main.resourceURL?
               ...
               FileManager.default.fileExists(atPath: auto.path) {
                ...
                appendOut("[ZForth agent] AutoLoad file not found in bundle\n")
            }
        }
        ...
        for step in parsed.steps {
            switch step {
            case .eval(let line):
                appendOut("[ZForth agent] eval: \(line)\n")
                let st = evaluateLine(line)
                appendOut("[ZForth agent] status=\(st) depth=\(zforth_agent_depth())\n")
                if st != 0 { failed = true }
            case .file(let path):
                appendOut("[ZForth agent] load: \(path)\n")
        ...
        if parsed.repl {
            appendOut("[ZForth agent] repl (stdin) — EOF to finish\n")
            while let line = readLine(strippingNewline: true) {
                let t = line.trimmingCharacters(in: .whitespacesAndNewlines)
                if t.isEmpty { continue }
                if t.uppercased() == "BYE" {
                    appendOut("[ZForth agent] BYE\n")
                    break
                }
        ...
        if parsed.dumpLast {
            appendOut("[ZForth agent] dump LAST body (64 bytes):\n")
            _ = evaluateLine("LAST")
            dumpLastBody(appendOut: appendOut)
        }

        if parsed.steps.isEmpty && !parsed.repl && !parsed.autoload && !parsed.dumpLast {
            appendOut("[ZForth agent] nothing to do (use -e, -f, --repl, or --autoload)\n")
            appendOut("Try: ZForth --agent --help\n")
            failed = true
        }

        appendOut(failed
            ? "[ZForth agent] DONE (failed)\n"
            : "[ZForth agent] DONE (ok)\n")
    ...
    /// Hex-dump dictionary cells at LAST's CFA (ITC header + body).
    private static func dumpLastBody(appendOut: (String) -> Void) {
        _ = evaluateLine("LAST")
        let n = zforth_agent_dump_tos_cfa(64)
        if n != 0 {
            appendOut("[ZForth agent] dump failed (\(n))\n")
        }
    }
    ...
    private static func evaluateInclude(path: String, appendOut: (String) -> Void) -> Int32 {
        ...
        guard !spec.isEmpty else {
            appendOut("[ZForth agent] ERROR: empty -f path\n")
            return 1
        }
        let abs: String
        if spec.hasPrefix("/") {
            ...
            default:
                if a.hasPrefix("-") {
                    FileHandle.standardError.write(
                        Data("[ZForth agent] unknown option: \(a)\n".utf8)
                    )
                }
                i += 1
            }
    ...
    private static func writeTranscriptIfNeeded(_ path: String?, _ text: String) -> Bool {
        ...
        do {
            ...
            let note = "[ZForth agent] transcript → \(path)\n"
            FileHandle.standardOutput.write(Data(note.utf8))
            return true
        } catch {
            let msg = "[ZForth agent] ERROR writing transcript \(path): \(error)\n"
    ...
    private static func printHelp() {
        let help = """
        ZForth agent channel — headless load / eval / capture

        ZForth --agent [options]
        ...
        Environment:
          ZFORTH_AGENT=1          Same as --agent (shell-safe name)

        Examples:
          ZForth --agent -e '2 2 + .'
          ZForth --agent -e ': 1+ 1 + ; 5 1+ .'
          ZForth --agent -c ~/proj -f smoke.fth -o /tmp/out.txt
          ZForth --agent --repl < session.txt
